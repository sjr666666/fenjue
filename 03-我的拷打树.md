...
> 对应简历两个项目：**多渠道获客数字员工平台（miniclaw）** + **蓬蓬AI · 员工超级助手（craft-agents）**
...
> 辰创（承接端，2026.02–04）→ 梵域（生产端，2026.05–08 · 八戒农牧）。两个 Agent 应用端由我独立负责，跑通"内容生产 → 多渠道触达 → 线索沉淀"获客闭环。
...
**Q：先一句话讲两段实习的关系？**
> 同一个获客闭环的两个端——先在深圳辰创做对客承接端（7 类 IM 数字员工，解决多渠道可靠接入），后到杭州梵域做内容生产端（蓬蓬AI 员工超级助手，落地农牧企业八戒农牧，解决多端多模型统一）。生产端产出经营材料/营销物料与话术 → 进入承接端完成触达分发 → 跟进结果沉淀画像 → bad case 回流两端迭代。口径：两段都是企业实习交付，应用端由我独立负责；Runtime 底座复用成熟开源（Pi），不升格。

**Q：两段实习怎么分工的？团队里你负责什么？**
> 辰创——获客承接端：渠道接入、三层身份模型、四账本可靠性、串行调度、画像沉淀，全部由我独立负责；梵域——内容生产端（八戒农牧）：AgentBackend 统一模型后端、AgentEvent 事件流、Electron/Web 统一 RPC、工具权限、MCP 与 8 大知识库接入。小团队交付，带教把关客户对接与架构评审（【模拟设定，A3 核实】）。
> 追问"实习生为什么能独立负责两个应用端"：小团队/小公司场景下应用端交付就是一个人扛——渠道适配、可靠性与权限这些"非模型"工程正是我的主战场；执行循环等成熟能力复用底座，把精力放在业务边界上。

**Q：实习期间遇到的最大技术难点？**
> 首选（生产端）双端渲染行为不一致：同一个错误 Web 能自动恢复、桌面端直接白屏——排查三步（复现 → 对比链路发现前端 API/IPC/后端 handler 各自演进已漂移 → 根因是事件无统一契约），修复用 AgentEvent 可辨识联合 + 契约测试进 CI。次选（承接端）多智能体会话污染：三层隔离 + 串行队列 + 双 revision 乐观锁。讲真事故，STAR 结构，复盘收获收尾。

**Q：八戒农牧这个客户，你具体交付了什么？**
> 蓬蓬AI 员工超级助手（内容生产端）：沉淀查/算/写/讲 4 大功能 × 6 部门共 72 个工作模板，接入养殖知识/疫病知识/饲养标准/政策法规等 8 大行业知识库（RAG 检索），覆盖员工超级助手与岗位专家双智能体。场景举例：场长桌面端发起饲料成本分析、月度经营汇报，切 Web 端继续实时查看；运营并行起草生猪行情日报、品牌宣传稿、政府项目申报材料，写盘先确认。使用规模按 00 A1/A2 口径，不编。
> 追问"72 个模板什么概念"：编码规则=功能（查/算/写/讲）× 部门（R1 经营管理~R6 生产）× 序号，4×6×3=72，每个有场景定位/典型输入/典型输出；首批 P0 上线 12 个（政府项目机会搜集/繁殖生产分析/饲料成本分析等）。

**Q：两段实习的技术能力怎么递进？**
> 承接端（先）："多人/多渠道怎么可靠"——IMChannel 统一渠道契约、四账本 + runId fencing、ACL 权限矩阵、/pair 绑定 + 信任阶梯。生产端（后）：复用并升级为"多端/多模型怎么统一"——AgentBackend、AgentEvent + 统一 RPC、Safe/Ask/Allow-All + 契约测试、8 大知识库 RAG。主线：可靠接入 → 统一抽象 → 「接入、执行、控险、稳跑」完整闭环。
...
> "我参与了两家企业实习交付（杭州梵域智能 · 八戒农牧 AI 工作平台 / 深圳辰创 · 获客数字员工平台），独立负责两个 Agent 应用端。第一个是多渠道获客数字员工平台，让 AI 数字员工长期驻场飞书、微信等 7 个 IM 渠道，自动承接客户触达、破冰、跟进，并把客户信息沉淀成结构化画像。第二个是 蓬蓬AI · 员工超级助手，面向农牧企业八戒农牧的市场与运营团队做桌面端和 Web 端复用的企业 AI 工作助手，沉淀查/算/写/讲 72 个工作模板、接入 8 大行业知识库，换模型、接企业系统都不改页面。两个项目共同跑通了'内容生产 → 多渠道触达 → 线索沉淀'的获客闭环。技术上我主要做了 Agent Runtime 工程化、多渠道统一接入与消息可靠性、模型多后端适配、MCP 能力接入和权限模型这些方面。"
